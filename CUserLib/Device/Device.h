...
public:
	...
	static Device *getDevice(CCommunicator* comm, int id, string name) {
		if (devicesByString.find(name) != devicesByString.end()) {
			return devicesByString[name];
		}
		return new Device(comm, id, name);
	}
	...
	Device(CCommunicator* comm, int id, string name);
	...
	void setComm(CCommunicator* comm);
